...
class RPINeopixel {
    static led(config) {
        if (rpi_detect_js_1.default.isPi5()) {
            winston_1.default.verbose(`RPINeopixel detected we are running on an RPi 5, returning SPI interface`);
            const led = new RPINeopixelLED_SPI(config);
            return led;
        }
        else {
            winston_1.default.verbose(`RPINeopixel detected we are not running on an RPi 5, returning ws281x interface`);
...
class RPINeopixelLED_SPI {
    ...
    constructor(config) {
        ...
        this._spi = pi_spi_1.default.initialize(spiInterface);
    }
